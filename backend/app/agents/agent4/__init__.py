...
A4 — Market Analyst
====================
Triggered by A5 (via Graph 4) after a candidate clears ALL interview rounds.

Steps:
  1. Read job details from Firestore (title, locations, budget, tech_stack)
  2. Search real salary data via Serper API (5 dynamic queries)
  3. Synthesize snippets with OpenAI GPT-4o-mini → P25 / P50 / P75 / P90
     Falls back to heuristic salary model if OpenAI is unavailable.
  4. Compute budget_warning (True if budget_max < P25)
  5. Persist SalaryReport to Firestore (job level + candidate level)
  6. Build manager-only email payload for A6
  7. Return updated CoordinationState

LLM: OpenAI GPT-4o-mini (OPENAI_API_KEY from .env — same key as Agent 1)
"""
...
# ── OpenAI client (lazy singleton — same pattern as Agent 1) ──────────────────
...
# ═══════════════════════════════════════════════════════════════════════════════
# Heuristic fallback — used when OpenAI is unavailable
# ═══════════════════════════════════════════════════════════════════════════════
...
# ═══════════════════════════════════════════════════════════════════════════════
# Serper salary search
# ═══════════════════════════════════════════════════════════════════════════════

def _search_salary_data(role_title: str, location: str) -> list[dict]:
    """Fire 5 targeted Serper queries — all inputs dynamic, year computed at runtime."""
    ...
    for query in queries:
        ...
        logger.info("[A4] Serper query=%r → %d results", query, len(hits))
        results.extend(hits)

    logger.info("[A4] Total Serper snippets: %d | role=%r location=%r year=%d",
                len(results), role_title, location, current_year)
    return results
...
def _default_analysis_summary(
    ...
    if budget_max > 0 and budget_max < p25:
        return (
            f"The {current_year} budget for {role_title} in {location} is below "
            f"market entry pay — the budget ceiling is under the P25 benchmark."
        )
    if budget_max >= p50:
        return (
            f"The {current_year} budget for {role_title} in {location} is competitive "
            f"— it reaches or exceeds the market median."
        )
    return (
...
def _fallback_salary_data(
    ...
    return {
        ...
        "analysis_summary": _default_analysis_summary(
            role_title, location, budget_min, budget_max, p25, p50,
        ),
    }


# ═══════════════════════════════════════════════════════════════════════════════
# OpenAI salary synthesis  (replaces Gemini)
# ═══════════════════════════════════════════════════════════════════════════════
...
def _synthesize_salary_data(
    ...
    """
    Feed Serper snippets to GPT-4o-mini → P25 / P50 / P75 / P90.
    Falls back to heuristic data if OpenAI is unavailable or returns bad JSON.
    Uses OPENAI_API_KEY from .env — same key as Agent 1.
    """
    api_key = os.environ.get("OPENAI_API_KEY")
    if not api_key:
        logger.info("[A4] OPENAI_API_KEY not set — using heuristic fallback")
    ...
    prompt = f"""You are a compensation analyst for a recruiting platform.
...
Return ONLY valid JSON with these exact keys (all salary values are annual USD integers):
{{
  "p25": <integer>,
  "p50": <integer>,
  "p75": <integer>,
  ...
  "analysis_summary": "<one sentence about whether the budget is competitive>"
}}
...
No markdown, no explanation, no extra keys — pure JSON only."""
    ...
    try:
        ...
        response = client.chat.completions.create(
            ...
            messages=[
                {
                    "role": "system",
                    "content": "You are a senior compensation analyst. Return only valid JSON.",
                },
        ...
        logger.warning("[A4] OpenAI synthesis failed, using heuristic fallback: %s", exc)
        return fallback_data
    ...
    p25, p50, p75, p90 = sorted([p25, p50, p75, p90])

    data_sources = payload.get("data_sources")
    if not isinstance(data_sources, list) or not data_sources:
        data_sources = fallback_data["data_sources"]
    else:
        data_sources = [str(s) for s in data_sources if str(s).strip()]
        if not data_sources:
            data_sources = fallback_data["data_sources"]
    ...
    logger.info("[A4] OpenAI synthesis: p25=%d p50=%d p75=%d p90=%d year=%d",
                p25, p50, p75, p90, current_year)
...
def _build_manager_email(
    ...
    if salary_report.budget_warning:
        budget_note = (
            f"⚠️  BUDGET WARNING: Company max (${budget_max:,.0f}) is BELOW "
            f"the market P25 (${salary_report.p25:,.0f}). "
            f"Consider revising the offer range."
        )
    else:
        budget_note = (
            f"The company budget (${budget_min:,.0f} – ${budget_max:,.0f}) "
            f"is within the {current_year} market range."
        )
    ...
    return {
        ...
        "body": (
            ...
            f"role in {location}, generated after {candidate_name} completed all "
            f"interview rounds.\n\n"
            f"── {current_year} Salary Benchmarks ─────────────────────────\n"
            f"  P25 (Entry / Low)    : ${salary_report.p25:,.0f}\n"
            f"  P50 (Median)         : ${salary_report.p50:,.0f}\n"
            f"  P75 (Senior / High)  : ${salary_report.p75:,.0f}\n"
            f"  P90 (Top of market)  : ${salary_report.p90:,.0f}\n\n"
            f"── Company Budget ────────────────────────────────────────────\n"
            f"  Range : ${budget_min:,.0f} – ${budget_max:,.0f}\n"
            f"  {budget_note}\n\n"
            f"── Market Summary ────────────────────────────────────────────\n"
            f"  {salary_report.analysis_summary or 'No summary available.'}\n\n"
            f"── Data Sources ──────────────────────────────────────────────\n"
            f"  {', '.join(salary_report.data_sources)}\n\n"
            f"Best regards,\nRecruitSquad AI"
        ),
...
# ═══════════════════════════════════════════════════════════════════════════════
# Main entry — run_market_analyst
# ═══════════════════════════════════════════════════════════════════════════════

async def run_market_analyst(state: CoordinationState) -> CoordinationState:
    """
    A4 entry — triggered by Graph 4 when next_action == "MARKET_ANALYSIS".

    Reads all inputs from Firestore (role, location, budget, tech_stack).
    Uses OpenAI GPT-4o-mini for salary synthesis (OPENAI_API_KEY from .env).
    Falls back to heuristic model if OpenAI is unavailable.
    """
    ...
    locations        = job.get("locations", ["Remote"]) or ["Remote"]
    primary_location = locations[0] if locations else "Remote"
    tech_stack       = list(job.get("tech_stack", []) or [])
    experience_range = (
    ...
    logger.info("[A4] Market analysis started: job=%s cid=%s role=%r location=%r",
                job_id, candidate_id, role_title, primary_location)

    # Serper search
    search_results = _search_salary_data(role_title, primary_location)

    # Heuristic fallback (always computed — used if OpenAI fails)
    fallback_data = _fallback_salary_data(
        role_title, primary_location, tech_stack,
        experience_range, budget_min, budget_max, search_results,
    )

    # OpenAI synthesis (replaces Gemini)
    salary_data = _synthesize_salary_data(
        role_title, primary_location, budget_min, budget_max,
    ...
    # Persist to Firestore — job level
    update_job(job_id, {
        "salary_report":  salary_report.model_dump(),
        "graph3_complete": True,
    })

    # Persist to Firestore — candidate level (if candidate_id present)
    ...
    if candidate_id:
        ...
        if candidate:
            ...
            logger.warning("[A4] Candidate %s not found under job %s", candidate_id, job_id)

    if salary_report.budget_warning:
        logger.warning("[A4] ⚠️  Budget $%g is BELOW market P25 $%g for %r",
                       budget_max, salary_data["p25"], role_title)
    else:
        logger.info("[A4] Budget $%g is competitive (P25=$%g)", budget_max, salary_data["p25"])

    state["salary_report"]    = salary_report
    state["graph3_complete"]  = True
    state["email_to_manager"] = _build_manager_email(
        role_title, primary_location, candidate_name,
        budget_min, budget_max, salary_report,
    )
    state["email_to_candidate"] = None   # candidate not emailed at this stage

    logger.info("[A4] Done. job=%s budget_warning=%s", job_id, salary_report.budget_warning)
    return state
